        ...
        */
        ...
        public string GerarDetalheRemessaCNAB400_A(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                /// Conforme manual CBNAB-400 da Sicredi, número sequencial do detalhe sempre informar 2
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0395, 006, 0, 2, '0'));                                         //395-400
                reg.CodificarLinha();
        ...
        public string GerarTrailerRemessa400(int numeroRegistro,  Cedente cedente)
        {
            try
            {
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0006, 005, 0, ObterCodigoBeneficiario(cedente), ' ')); //006-010                
        ...
        public string ObterCodigoBeneficiario(Cedente cedente) 
            => cedente.ContaBancaria?.Conta.Split('-').FirstOrDefault() ?? cedente.Codigo;
    }
}
